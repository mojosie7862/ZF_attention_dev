...
path = r'C:\Users\josep\anaconda3\envs\ZF_attention_project\paradigms.pptx'
...
def main_recording(ind, f_times):
    if (toggle == 1):
        index = -999
        ...
        with open("transcript.txt", "a+") as wfile2:
            wfile2.write("Run # " + str(ind + 1) + "\n")

        recordVideo(index+1, ind+1, f_times)

def main_ppt(app, a_run, f_times):

    win32api.Sleep(pre_stimulus_time * 1000)  # pre-stimulus time
    app.SlideShowWindows(1).View.GotoSlide(a_run[1])  # advance to screen cue
...
def main_():
    ...
    recordVideo(index+1, numruns, fixed_times, novel=True)    #novel environment test

    for i in range(numruns):
        this_run = random.choice(paradigm_slides)
        print('run', i + 1, ':', this_run[0], 'ITI:', iti)

        p = threading.Thread(target=main_ppt(x, this_run, fixed_times))
        r = threading.Thread(target=main_recording(i, fixed_times))
        r.start()
        time.sleep(pre_stimulus_time)
        p.start()
...
def recordVideo(recordingIndex, max, f_times, novel=False):
    try:
        ...
        if novel:
            endtime = 5 #switch to 360 for 6 mins - 5 for testing
        else:
            endtime = sum(f_times) / 1000
            endtime = endtime + int(pre_stimulus_time) + int(pre_reward_time) + int(reward_aversion_time)
